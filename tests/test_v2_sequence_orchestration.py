...
class FakeClip:
    ...
    def tokenize(self, prompt, images):
        self.events.append(("tokenize", prompt, len(images)))
        return prompt
...
def test_v2_samples_every_chunk_before_any_decode(monkeypatch):
    ...
    def clone_model_for_chunk(
        ...
        *,
        ...
        if context_frames is not None:
            transformer_options[CONTINUUM_INTEROP_KEY] = {
                "api": 1,
                "active": True,
    ...
    def sample_chunk(**kwargs):
        frames = kwargs["latent"]["samples"].unbind()[0].shape[2]
        hint = kwargs["model"].model_options["transformer_options"].get(
            CONTINUUM_INTEROP_KEY
        )
        events.append(("sample", int(frames), kwargs["seed"], hint))
        return kwargs["latent"]

    def decode_sequence(**kwargs):
        events.append(("decode", len(kwargs["entries"])))
        images = torch.zeros(362, 2, 2, 3)
    ...
    monkeypatch.setattr(sequence, "decode_sequence", decode_sequence)

    def unexpected_seam_decode(**kwargs):
        raise AssertionError("Off must not enter the V2.1 seam decoder")

    monkeypatch.setattr(sequence, "decode_sequence_with_seam", unexpected_seam_decode)
    ...
    images, audio, last_state, session, report = sequence.run_sequence(
        model=fake_model,
        clip=FakeClip(events),
        video_vae=object(),
        audio_vae=object(),
        sampler=object(),
    ...
    sample_positions = [i for i, event in enumerate(events) if event[0] == "sample"]
    decode_position = next(i for i, event in enumerate(events) if event[0] == "decode")
    assert len(sample_positions) == 3
    assert max(sample_positions) < decode_position
    sample_events = [event for event in events if event[0] == "sample"]
    assert sample_events[0][3] is None
    assert sample_events[1][3]["min_actual_prefix_steps"] == 2
    assert sample_events[2][3]["min_actual_prefix_steps"] == 2
    clone_events = [event for event in events if event[0] == "clone"]
    assert [event[2] for event in clone_events] == [None, 22, 22]
    # Fixed mode encodes its shared text conditioning only once.
    assert sum(event[0] == "encode" for event in events) == 1
