...
def test_chunk_model_copy_on_write_and_context_scoped_hint(monkeypatch):
    ...
    base = patch_model(source, strict=True, debug=False)
    first = clone_model_for_chunk(
        base,
        strict=True,
        debug=False,
        chunk_index=1,
    ...
    assert CONTINUUM_INTEROP_KEY not in base.model_options["transformer_options"]
