...
| 動画 | 実行 | 生成時間 | 結果 |
|---|---|---:|---|
| 00009 | 初回生成 | 1073.81秒 | 0 reused / 3 generated、manifest complete、resume_safe=true、3 Chunk SHA-256一致 |
| 00012 | 完全Auto Resume | 116.86秒 | 3 reused / 0 generated、resume=complete、Ref2VA verified |

最終判定:

- Python構文確認、pytest 129件、GitHub Actions、ローカル100ファイル一致を確認。
- Reference + Continuation、外部Core VAE Decode、Spectrum Actual Prefix 2、Run Storage保存と完全再開をWindows実機で確認。
- FL2VA/Ref2VA、Prompt、解像度、Run Storage、アップスケール経路、Reference構成の変更を別条件として識別した。
- 異なる条件のChunkを誤再利用した事例はない。
- V3.2.1を正式安定版として採用する。
