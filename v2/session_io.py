...
def sanitize_prefix(prefix: str) -> str:
    ...
    return value or "h3_continuum_session"
...
def session_directory() -> Path:
    try:
        ...
        root = Path.cwd()
    path = root / "h3_continuum_sessions"
...
def session_paths(prefix: str, slot: int) -> tuple[Path, Path]:
    slot = int(slot)
    if not (1 <= slot <= 9999):
        raise ValueError("session slot must be between 1 and 9999")
...
def _metadata_without_tensors(session: dict[str, Any]) -> dict[str, Any]:
    metadata = {key: value for key, value in session.items() if key != "chunks"}
    chunks = []
    for entry in session["chunks"]:
        chunks.append({key: value for key, value in entry.items() if key not in ("video", "audio")})
    metadata["chunks"] = chunks
    return metadata


def save_session(session: dict[str, Any], *, prefix: str, slot: int) -> tuple[Path, Path]:
    ...
    for index, entry in enumerate(session["chunks"], start=1):
        tensors[f"chunk_{index:04d}_video"] = entry["video"].detach().to("cpu").contiguous()
        tensors[f"chunk_{index:04d}_audio"] = entry["audio"].detach().to("cpu").contiguous()
    metadata = _metadata_without_tensors(session)
    metadata["_session_commit_id"] = token
...
def load_session(*, prefix: str, slot: int) -> dict[str, Any]:
    tensor_path, json_path = session_paths(prefix, slot)
    if not tensor_path.is_file():
        raise FileNotFoundError(f"session slot does not exist: {tensor_path.name}")
    with safe_open(str(tensor_path), framework="pt", device="cpu") as handle:
        serialized = (handle.metadata() or {}).get(_HEADER_KEY)
        if serialized is None:
            raise ValueError("session safetensors has no embedded Continuum metadata")
        session = json.loads(serialized)
        for index, entry in enumerate(session.get("chunks") or [], start=1):
            entry["video"] = handle.get_tensor(f"chunk_{index:04d}_video")
            entry["audio"] = handle.get_tensor(f"chunk_{index:04d}_audio")

    if json_path.is_file():
        try:
            with json_path.open("r", encoding="utf-8") as handle:
                mirror = json.load(handle)
            if mirror.get("_session_commit_id") != session.get("_session_commit_id"):
                LOG.warning("Ignoring stale Continuum session JSON mirror: %s", json_path.name)
        except (OSError, ValueError, TypeError) as exc:
            LOG.warning("Ignoring unreadable session JSON mirror %s: %s", json_path.name, exc)
    return validate_session(session)


def session_mtime(*, prefix: str, slot: int) -> tuple[float, float]:
    tensor_path, json_path = session_paths(prefix, slot)
    return (
