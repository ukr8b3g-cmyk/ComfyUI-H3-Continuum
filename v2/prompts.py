...
_CHUNK_HEADER = re.compile(
    r"^\s*\[\s*(?:chunk|clip)\s*(?P<index>\d+)\s*\]\s*$",
    re.IGNORECASE,
)
...
def _normalize_prompt(value: str, *, label: str) -> str:
    ...
    if not text:
        raise PromptPlanError(f"{label} is empty")
    return text


def _parse_json_list(script: str) -> list[str] | None:
    ...
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        raise PromptPlanError("JSON prompt list must be an array of strings")
    return [_normalize_prompt(item, label=f"prompt {index + 1}") for index, item in enumerate(value)]


def _parse_list(script: str, chunks: int) -> tuple[list[str], list[str]]:
    ...
    if not values:
        raise PromptPlanError("prompt list contains no prompts")
    notes: list[str] = []
...
def _parse_timeline_sections(script: str) -> list[dict[str, Any]]:
        ...
        if current is None:
            if any(line.strip() for line in body):
                raise PromptPlanError(
                    "timeline text before the first [0-5s] or [Chunk 1] header is not allowed"
                )
            body = []
            return
    ...
    for line in str(script).splitlines():
        ...
        if time_match or chunk_match:
            ...
            if time_match:
                ...
                if not end > start:
                    raise PromptPlanError(f"timeline range must increase: {line.strip()}")
                current = {"kind": "time", "start": start, "end": end}
            else:
                index = int(chunk_match.group("index"))
                if index < 1:
                    raise PromptPlanError("chunk numbers are one-based")
...
def _parse_timeline(script: str, chunks: int, chunk_seconds: float) -> tuple[list[str], list[str]]:
    sections = _parse_timeline_sections(script)
    prompts: list[str] = []
    notes: list[str] = []
    for chunk_index in range(1, chunks + 1):
        chunk_sections = [
            item for item in sections if item["kind"] == "chunk" and item["index"] == chunk_index
        ]
        if len(chunk_sections) > 1:
            raise PromptPlanError(f"timeline defines Chunk {chunk_index} more than once")
        if chunk_sections:
            prompts.append(chunk_sections[0]["prompt"])
            continue
        ...
        if not scored:
            raise PromptPlanError(
                f"timeline does not cover chunk {chunk_index} ({start:.3f}-{end:.3f}s)"
            )
        scored.sort(reverse=True, key=lambda row: (row[0], row[1]))
        prompts.append(scored[0][2]["prompt"])
...
def detect_prompt_mode(script: str) -> str:
    """Detect the stable Prompt Plan mode without changing the input text."""
...
def resolve_prompt_mode(mode: str, script: str) -> str:
    explicit = {
...
def make_prompt_plan(
    *, mode: str, script: str, chunks: int, chunk_seconds: float,
) -> dict[str, Any]:
    chunks = int(chunks)
    ...
    if mode == PROMPT_FORMAT_AUTO:
        detected = {
            PROMPT_MODE_FIXED: PROMPT_FORMAT_FIXED,
            PROMPT_MODE_LIST: PROMPT_FORMAT_LIST,
            PROMPT_MODE_TIMELINE: PROMPT_FORMAT_TIMELINE,
        }[resolved_mode]
    ...
    if resolved_mode == PROMPT_MODE_FIXED:
        ...
        prompts, parse_notes = _parse_list(script, chunks)
        notes.extend(parse_notes)
    elif resolved_mode == PROMPT_MODE_TIMELINE:
        prompts, parse_notes = _parse_timeline(script, chunks, chunk_seconds)
        notes.extend(parse_notes)
    else:
    ...
    return {
        ...
        "prompts": prompts,
        "hashes": [prompt_hash(prompt) for prompt in prompts],
        "notes": notes,
    }


def validate_prompt_plan(plan: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(plan, dict) or plan.get("magic") != PROMPT_PLAN_MAGIC:
        raise PromptPlanError("invalid H3 Continuum prompt plan")
    if int(plan.get("schema_version", -1)) != PROMPT_PLAN_SCHEMA_VERSION:
        raise PromptPlanError(
            f"unsupported prompt-plan schema {plan.get('schema_version')}; "
            f"expected {PROMPT_PLAN_SCHEMA_VERSION}"
        )
...
def build_sampler_prompt_plan(
    *,
    prompt_mode: str,
    prompt_script: str,
    sequence_prompt: str | None,
    prompt_plan: dict[str, Any] | None,
    chunks: int,
    chunk_seconds: float,
) -> dict[str, Any]:
    """Resolve the fixed Sequence socket while preserving legacy fallbacks."""
    ...
    if sequence_prompt is not None:
        return make_prompt_plan(
            mode=prompt_mode,
            script=sequence_prompt,
            chunks=chunks,
            chunk_seconds=chunk_seconds,
        )
    if prompt_plan is not None:
        plan = validate_prompt_plan(prompt_plan)
        if int(plan["chunks"]) != chunks:
            raise PromptPlanError(
                "connected prompt_plan chunks do not match the Sampler chunks widget"
            )
        if abs(float(plan["chunk_seconds"]) - chunk_seconds) > 1e-6:
            raise PromptPlanError(
                "connected prompt_plan chunk_seconds do not match the Sampler widget"
            )
        return plan
    return make_prompt_plan(
        mode=prompt_mode,
...
def apply_prompt_overrides(
    plan: dict[str, Any], overrides: list[str | None]
) -> dict[str, Any]:
    """Replace selected chunk prompts without changing the prompt-plan schema."""
    ...
    for index, value in enumerate(overrides[: int(plan["chunks"])]):
        if value is None:
            continue
        prompts[index] = _normalize_prompt(value, label=f"Clip {index + 1} Prompt")
    ...
    result["hashes"] = [prompt_hash(prompt) for prompt in prompts]
    result["notes"] = list(plan.get("notes") or []) + [
        "external Clip Prompt input(s): " + ", ".join(map(str, replaced))
    ]
    return validate_prompt_plan(result)


def prompt_plan_report(plan: dict[str, Any]) -> str:
    plan = validate_prompt_plan(plan)
    unique = len(set(plan["hashes"]))
    note = "; ".join(plan.get("notes") or [])
    result = (
        f"Prompt plan: {plan['mode']}; {plan['chunks']} chunks; "
        f"{unique} unique prompt(s); {plan['chunk_seconds']:.3f}s per chunk."
    )
