...
def latent_slot_offsets(latent_t: int) -> tuple[int, ...]:
    ...
    for index in range(int(latent_t)):
        out.append(cursor)
        cursor += FRAME_PER_TOKEN[index % 5]
    return tuple(out)
...
def context_slots(context_frames: int) -> int:
    ...
    for slots in range(1, 256):
        ...
        if frames > target:
            break
    raise ValueError(
        f"context_frames={context_frames} is not representable; use 5, 22, or 39"
    )
...
def audio_grid_offset(frame_count: int, actual_audio_t: int) -> float:
    """Signed audio-grid end offset in 40 Hz latent steps.

    Native H3 uses ``round(duration * 40)``. Depending on the valid 17k+5
    frame count, the audio grid can end up to half a latent step before or
    after the exact video end. Keeping the sign is required for sample-accurate
...
# Backward-compatible public name used by state schema v1.
...
def audio_latent_t_from_grid_offset(frame_count: int, grid_offset: float) -> int:
    """Recover the actual source audio-grid end without guessing its phase."""
    ...
    if abs(recovered - float(grid_offset)) > 1e-6:
        raise ValueError(
            "source audio-grid phase cannot be recovered from frame count and signed offset"
        )
    return candidate


def make_av_context_window(
    source_frame_count: int,
    actual_audio_t: int,
    context_frames: int,
) -> AVContextWindow:
    """Resolve a raw AV tail on the native 5-step video and 40/24 audio grids."""

    source_frames = int(source_frame_count)
    context = int(context_frames)
    if not is_valid_frame_count(source_frames):
        raise ValueError(
            f"source_frame_count={source_frames} is not on the native H3 17k+5 grid"
        )
    ...
    source_video_t = video_latent_t(source_frames)
    context_video_t = context_slots(context)
    video_start = source_video_t - context_video_t
    if video_start < 0 or video_start % len(FRAME_PER_TOKEN):
        raise ValueError(
            "video context tail does not begin at a uniquely recoverable H3 latent-cycle phase"
        )

    audio_stop = int(actual_audio_t)
    grid_offset = audio_grid_offset(source_frames, audio_stop)
    if not (-0.500001 <= grid_offset <= 0.500001):
        raise ValueError(f"source audio-grid offset {grid_offset:.6f} is outside [-0.5,0.5]")
    audio_steps = audio_latent_t(context)
    audio_start = audio_stop - audio_steps
    if audio_start < 0:
        raise ValueError("source audio latent is too short for the requested context")

    return AVContextWindow(
        source_frame_count=source_frames,
        context_frames=context,
        video_start_slot=video_start,
        video_stop_slot=source_video_t,
        audio_start_tick=audio_start,
        audio_stop_tick=audio_stop,
...
def make_extension_shape(context_frames: int, extend_seconds: float) -> ExtensionShape:
    ...
    total = align_frame_count_nearest(
        context_frames + requested_new, minimum=context_frames + 1
    )
    net = total - context_frames
    if net < 1:
        raise RuntimeError("computed continuation contains no new frames")
    return ExtensionShape(
        context_frames=context_frames,
        requested_new_frames=requested_new,
        total_frames=total,
        net_new_frames=net,
        video_latent_t=video_latent_t(total),
        audio_latent_t=audio_latent_t(total),
    )


def largest_context_capacity(frame_count: int) -> int:
    """Largest supported continuation profile contained in ``frame_count``."""
    ...
    for capacity in (39, 22, 5):
        if available >= capacity:
            return capacity
    raise ValueError(
        f"at least 5 retained frames are required for a continuation state; got {available}"
    )
...
def run_temporal_self_test() -> None:
    expected = {
        5: (2, (0, 1)),
        22: (7, (0, 1, 5, 9, 13, 17, 18)),
        39: (12, (0, 1, 5, 9, 13, 17, 18, 22, 26, 30, 34, 35)),
    }
